...
<table width="100%" cellpadding="5" cellspacing="1">
          ...
          <tr bgcolor="#96CA1D" id="entityRowOdd">
            ...
            <td width="479">            <input name="name" type="text" id="name"  value="<?php if($action=='update'){echo $fetilizername;} ?>" size="55"/>
            <input type="hidden" name="id" id="id" value="<?php if($action=='update'){echo $recID;} ?>" /></td>
          </tr>
          
          <tr id="entityRowEven">
            <td>Description</td>
            <td><textarea name="description" cols="42" rows="3" id="description" $description="$description"><?php if($action=='update'){echo $description;} ?>
            </textarea></td>
          </tr>
          <tr id="entityRowEven">
            <td>Timing</td>
            <td><textarea name="timing" cols="42" rows="3" id="timing" $description="$description"><?php if($action=='update'){echo $description;} ?>
            </textarea></td>
          </tr>
          <tr id="entityRowEven">
            <td>Dosage</td>
            <td><textarea name="dosage" cols="42" rows="3" id="dosage" $description="$description"><?php if($action=='update'){echo $description;} ?>
            </textarea></td>
          </tr>
